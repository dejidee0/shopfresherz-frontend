'use client'

import { useState } from 'react'
import Link from 'next/link'
import Image from 'next/image'
import { FiChevronRight, FiX } from 'react-icons/fi'
import { FiShoppingCart } from 'react-icons/fi'
import { PriceRangeSlider } from '@/components/ui/PriceRangeSlider'
import { cn, formatPrice } from '@/lib/utils/format'

// ─── Types ────────────────────────────────────────────────────────────────────

export interface ShopFilters {
  categorySlug: string | null
  priceRange: [number, number]
  brands: string[]
  tags: string[]
}

interface ShopSidebarProps {
  filters: ShopFilters
  onChange: (filters: ShopFilters) => void
}

// ─── Static data (swap with API when ready) ───────────────────────────────────

const CATEGORIES = [
  { label: 'Computer & Laptop', slug: 'computer-laptop', children: [
    { label: 'Computer Accessories', slug: 'computer-accessories' },
    { label: 'Smartphone', slug: 'mobile-phones' },
    { label: 'Headphone', slug: 'accessories/headphones' },
    { label: 'Mobile Accessories', slug: 'accessories/mobile' },
    { label: 'Gaming Console', slug: 'games-consoles' },
    { label: 'Camera & Photo', slug: 'electronics/cameras' },
    { label: 'Romoss', slug: 'romoss' },
    { label: 'TV & Homes Appliances', slug: 'electronics/tv' },
    { label: 'Watchs & Accessories', slug: 'smart-watches' },
    { label: 'GPS & Navigation', slug: 'gps-navigation' },
    { label: 'Warable Technology', slug: 'wearables' },
  ]},
]

const PRICE_PRESETS: { label: string; range: [number, number] }[] = [
  { label: 'All Price', range: [0, 80_000] },
  { label: 'Under ₦5,000', range: [0, 5_000] },
  { label: '₦5,000 to ₦10,000', range: [5_000, 10_000] },
  { label: '₦10,000 to ₦20,000', range: [10_000, 20_000] },
  { label: '₦20,000 to ₦30,000', range: [20_000, 30_000] },
  { label: '₦30,000 to ₦40,000', range: [30_000, 40_000] },
  { label: '₦50,000 to ₦80,000', range: [50_000, 80_000] },
]

const BRANDS = [
  'Apple', 'Microsoft', 'Dell', 'Symphony', 'Sony', 'LG', 'One Plus',
  'Google', 'Samsung', 'HP', 'Xiaomi', 'Panasonic', 'Intel',
]

const POPULAR_TAGS = [
  'Game', 'iPhone', 'TV', 'Asus Laptops',
  'Macbook', 'SSD', 'Graphics Card',
  'Power Bank', 'Smart TV', 'Speaker',
  'Tablet', 'Microwave', 'Samsung',
]

// ─── Featured promo product (bottom of sidebar) ───────────────────────────────
const PROMO = {
  tag: 'Apple',
  name: 'WATCH SERIES 7',
  headline: 'Heavy on Features.',
  subtext: 'Light on Price.',
  label: 'Only for:',
  price: 299,
  currency: 'USD',
  image: '/images/promo/watch-series7.png',
  slug: 'apple-watch-series-7',
}

// ─── Section shell ────────────────────────────────────────────────────────────

function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="border-b border-[#F5F5F5] pb-5 mb-5 last:border-none last:mb-0">
      <h3 className="text-sm font-bold text-[#111111] uppercase tracking-wide mb-3">
        {title}
      </h3>
      {children}
    </div>
  )
}

// ─── Main sidebar ─────────────────────────────────────────────────────────────

export function ShopSidebar({ filters, onChange }: ShopSidebarProps) {
  function set<K extends keyof ShopFilters>(key: K, value: ShopFilters[K]) {
    onChange({ ...filters, [key]: value })
  }

  function toggleBrand(brand: string) {
    const next = filters.brands.includes(brand)
      ? filters.brands.filter((b) => b !== brand)
      : [...filters.brands, brand]
    set('brands', next)
  }

  function toggleTag(tag: string) {
    const next = filters.tags.includes(tag)
      ? filters.tags.filter((t) => t !== tag)
      : [...filters.tags, tag]
    set('tags', next)
  }

  return (
    <aside className="w-60 shrink-0 space-y-0">

      {/* ── Category ── */}
      <Section title="Category">
        <ul className="space-y-0.5">
          {CATEGORIES.map((cat) => (
            <li key={cat.slug}>
              {/* Parent */}
              <button
                onClick={() => set('categorySlug', cat.slug)}
                className={cn(
                  'flex items-center gap-2 w-full text-left text-sm py-1.5 px-1 rounded transition-colors',
                  filters.categorySlug === cat.slug
                    ? 'text-[#F5820A] font-semibold'
                    : 'text-[#111111] hover:text-[#F5820A]'
                )}
              >
                <span
                  className={cn(
                    'w-3.5 h-3.5 rounded-full border-2 shrink-0 transition-colors',
                    filters.categorySlug === cat.slug
                      ? 'border-[#F5820A] bg-[#F5820A]'
                      : 'border-[#D1D5DB]'
                  )}
                />
                {cat.label}
              </button>

              {/* Children */}
              <ul className="ml-5 space-y-0.5">
                {cat.children.map((child) => (
                  <li key={child.slug}>
                    <button
                      onClick={() => set('categorySlug', child.slug)}
                      className={cn(
                        'flex items-center gap-2 w-full text-left text-sm py-1 px-1 rounded transition-colors',
                        filters.categorySlug === child.slug
                          ? 'text-[#F5820A] font-semibold'
                          : 'text-[#6B7280] hover:text-[#F5820A]'
                      )}
                    >
                      <span
                        className={cn(
                          'w-3 h-3 rounded-full border-2 shrink-0 transition-colors',
                          filters.categorySlug === child.slug
                            ? 'border-[#F5820A] bg-[#F5820A]'
                            : 'border-[#D1D5DB]'
                        )}
                      />
                      {child.label}
                    </button>
                  </li>
                ))}
              </ul>
            </li>
          ))}
        </ul>
      </Section>

      {/* ── Price Range ── */}
      <Section title="Price Range">
        <PriceRangeSlider
          min={0}
          max={80_000}
          value={filters.priceRange}
          onChange={(range) => set('priceRange', range)}
        />

        <ul className="space-y-1 mt-3">
          {PRICE_PRESETS.map(({ label, range }) => {
            const isActive =
              filters.priceRange[0] === range[0] && filters.priceRange[1] === range[1]
            return (
              <li key={label}>
                <button
                  onClick={() => set('priceRange', range)}
                  className={cn(
                    'flex items-center gap-2 w-full text-left text-sm py-0.5 transition-colors',
                    isActive ? 'text-[#F5820A] font-medium' : 'text-[#6B7280] hover:text-[#F5820A]'
                  )}
                >
                  <span
                    className={cn(
                      'w-3 h-3 rounded-full border-2 shrink-0',
                      isActive ? 'border-[#F5820A] bg-[#F5820A]' : 'border-[#D1D5DB]'
                    )}
                  />
                  {label}
                </button>
              </li>
            )
          })}
        </ul>
      </Section>

      {/* ── Popular Brands ── */}
      <Section title="Popular Brands">
        <div className="grid grid-cols-2 gap-x-4 gap-y-1.5">
          {BRANDS.map((brand) => {
            const checked = filters.brands.includes(brand)
            return (
              <label
                key={brand}
                className="flex items-center gap-2 cursor-pointer group"
              >
                <span
                  onClick={() => toggleBrand(brand)}
                  className={cn(
                    'w-4 h-4 rounded border-2 shrink-0 flex items-center justify-center transition-colors',
                    checked
                      ? 'bg-[#F5820A] border-[#F5820A]'
                      : 'border-[#D1D5DB] group-hover:border-[#F5820A]'
                  )}
                >
                  {checked && (
                    <svg width="9" height="7" viewBox="0 0 9 7" fill="none">
                      <path d="M1 3.5L3.5 6L8 1" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                  )}
                </span>
                <span
                  onClick={() => toggleBrand(brand)}
                  className={cn(
                    'text-sm transition-colors',
                    checked ? 'text-[#F5820A] font-medium' : 'text-[#6B7280] group-hover:text-[#111111]'
                  )}
                >
                  {brand}
                </span>
              </label>
            )
          })}
        </div>
      </Section>

      {/* ── Popular Tags ── */}
      <Section title="Popular Tag">
        <div className="flex flex-wrap gap-2">
          {POPULAR_TAGS.map((tag) => {
            const active = filters.tags.includes(tag)
            return (
              <button
                key={tag}
                onClick={() => toggleTag(tag)}
                className={cn(
                  'px-2.5 py-1 text-xs rounded border transition-all duration-150',
                  active
                    ? 'bg-[#F5820A] border-[#F5820A] text-white'
                    : 'border-[#E5E7EB] text-[#6B7280] hover:border-[#F5820A] hover:text-[#F5820A]'
                )}
              >
                {tag}
              </button>
            )
          })}
        </div>
      </Section>

      {/* ── Promo card ── */}
      <div className="bg-white border border-[#E5E7EB] rounded-card overflow-hidden">
        {/* Product image */}
        <div className="bg-[#F5F5F5] aspect-square flex items-center justify-center p-4">
          <Image
            src={PROMO.image}
            alt={PROMO.name}
            width={160}
            height={160}
            className="object-contain w-full h-full"
          />
        </div>

        {/* Text */}
        <div className="p-4 text-center">
          <p className="text-xs font-bold text-[#111111] uppercase tracking-wide mb-0.5">
            {PROMO.tag}
          </p>
          <p className="text-base font-extrabold text-[#111111] leading-tight">{PROMO.name}</p>
          <p className="text-sm font-bold text-[#111111] mt-1">{PROMO.headline}</p>
          <p className="text-sm text-[#6B7280]">{PROMO.subtext}</p>

          <div className="mt-3 flex items-center justify-center gap-1.5 text-sm text-[#6B7280]">
            <span>{PROMO.label}</span>
            <span className="font-bold text-base text-[#111111]">${PROMO.price} {PROMO.currency}</span>
          </div>

          <div className="flex flex-col gap-2 mt-4">
            <Link
              href={`/product/${PROMO.slug}`}
              className="flex items-center justify-center gap-2 h-9 bg-linear-to-r from-[#F5820A] to-[#E06B00] text-white text-xs font-semibold rounded-btn hover:shadow-md transition-all"
            >
              <FiShoppingCart size={13} />
              ADD TO CART
            </Link>
            <Link
              href={`/product/${PROMO.slug}`}
              className="flex items-center justify-center gap-2 h-9 border border-[#E5E7EB] text-[#111111] text-xs font-semibold rounded-btn hover:border-[#F5820A] hover:text-[#F5820A] transition-all"
            >
              VIEW DETAILS →
            </Link>
          </div>
        </div>
      </div>
    </aside>
  )
}