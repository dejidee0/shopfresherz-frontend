...
//                 cardType === t.value
//                   ? 'border-[#F5820A] text-[#F5820A] bg-orange-50'
//                   : 'border-[#E5E7EB] text-[#6B7280] hover:border-[#F5820A]/40'
//               }`}
//             >
//               {t.label}
//             </button>
...
//           className="w-4 h-4 accent-[#F5820A] rounded"
//         />
...
//         className="self-start text-xs font-bold px-4 py-2 rounded-lg bg-[#F5820A] text-white hover:bg-[#E06B00] transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
//       >
...
import { accountApi } from '@/lib/api/account'
import { useAuthStore } from '@/store/auth'
import { SAVED_CARDS_QUERY_KEY } from '@/app/store/checkout/page'
...
function CardFormFields({
  ...
  return (
    <div className="border border-[#E5E7EB] rounded-lg p-4 mt-3 flex flex-col gap-3 bg-[#FAFAFA]">
      ...
      {errors.submit && (
        <p className="text-xs text-red-500 bg-red-50 rounded-lg px-3 py-2">{errors.submit}</p>
      )}
      {saved && (
        <p className="text-xs text-green-600 bg-green-50 rounded-lg px-3 py-2">
